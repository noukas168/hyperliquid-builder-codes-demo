...
**The mark** is the Spread: two horizontal bars, offset across and down, with a
diagonal gap between them. The gap is the point of it, and its size is set by
what survives a 16px favicon rather than by what looks best at 512px. Assets
live in `public/brand/` and are generated by
`scripts/generate-brand.mjs`, which holds the geometry and is the place to
change it. `mark.svg` and the `BasisMark` component use `currentColor`, so the
mark takes the brand token rather than a copy of its value; the component is
what the header renders, because an `<img>` cannot inherit a colour. Regenerate
with `node scripts/generate-brand.mjs` after any change, since the `.ico` and
`.png` cannot be hand-edited.
